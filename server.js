...
app.get("/admin", (req, res) => {
    req.session.user = "glemtPassord";
    res.send({ user: "glemtPassord", login: true });
});
...
app.get("/rmUser/:username", async (req, res) => {
    if (req.session.hasOwnProperty("user")) {
        await Users.destroy({ where: { name: req.params.username } });
        res.send({ login: true });
    } else {
        res.send({ login: false });
    }
...
app.get("/rmStudent/:username", async (req, res) => {
    if (req.session.hasOwnProperty("user")) {
        await Elever.destroy({ where: { name: req.params.username } });
        res.send({ login: true });
    } else {
        res.send({ login: false });
    }
...
app.get("/rmTeacher/:username", async (req, res) => {
    if (req.session.hasOwnProperty("user")) {
        Lærere.destroy({ where: { name: req.params.username } });
        res.send({ login: true });
    } else {
        res.send({ login: false });
    }
